...
struct mtqp {
  ...
  uint64_t* totscnt;
  uint64_t* totccnt;
//	uint64_t* scnt_for_qp;

  int return_value;
};
pthread_mutex_t  mutex = PTHREAD_MUTEX_INITIALIZER;
struct mtqp mtqp_arg;
pthread_mutex_t* mutex_arr;
//struct timeval end_t;
double diff;

atomic_ullong	totscnt = 0;
#define MING_SEED 950514
...
#endif
// for debugging
void print_status(struct pingpong_context *ctx, int num_of_qps, char* expl)
{
  printf("[%s]\n", expl);
  for (int index=0; index<num_of_qps; index++){
    printf("    [#%d] scnt: %d | ccnt: %d\n", index, ctx->scnt[index], ctx->ccnt[index]);
  }
}

void print_qps(struct pingpong_context *ctx, struct perftest_parameters *user_param)
{
  // MUST Run on main thread ONLY
  gettimeofday(&ctx->end_t, NULL);
  diff = ( ctx->end_t.tv_sec - ctx->start_t.tv_sec ) + (( ctx->end_t.tv_usec - ctx->start_t.tv_usec ) / 1000000);
  
  if (diff >= user_param->duration){
    ...
    gettimeofday(&ctx->start_t, NULL);
  }
  
}

void *mtqp_post(struct mtqp* arg)
{
  struct pingpong_context *ctx = arg->ctx;
  struct perftest_parameters *user_param = arg->user_param;
  int index;
  int tid = arg->index;
  int* idx_lst = arg->idx_lst;
...
*/
  ...
  while (1){
    //print_status(ctx, user_param->num_of_qps, "mt_qp_post");
    for (int idx =0 ; idx < user_param->num_of_qps ; idx++) {
      index = idx_lst[idx];
      if((ctx->scnt[index] - ctx->ccnt[index]) < user_param->tx_depth)
      {
        if(pthread_mutex_trylock(&(mutex_arr[index])) == 0)
        {
          while ((ctx->scnt[index] - ctx->ccnt[index]) < user_param->tx_depth) {
            ...
            if (err) {
              ...
              arg->return_value = FAILURE;
              pthread_exit(NULL);
            }
            ctx->scnt[index] += user_param->post_list;
            ctx->scnt_now[index] += user_param->post_list;
...
void *mtqp_post2(struct mtqp* arg)
{
  struct pingpong_context *ctx = arg->ctx;
  struct perftest_parameters *user_param = arg->user_param;
  int tid = arg->index;
...
/*  printf("Thread #%d index list: ", tid);
  for (int i=0; i<arg->idx_len; i++){
    printf("%d ", idx_lst[i]);
  }
  printf("\n");
*/
...
*/
  // Assign one thread for each qp
  while (1){
    for (int idx =0 ; idx < arg->idx_len ; idx++) {
      index = idx_lst[idx];
      //if (index < 0) continue;
      while ((ctx->scnt[index] - ctx->ccnt[index]) < user_param->tx_depth) {
        if (user_param->post_list == 1 && (ctx->scnt[index] % user_param->cq_mod == 0 && user_param->cq_mod > 1)) {
...
void alloc_ctx(struct pingpong_context *ctx,struct perftest_parameters *user_param)
{
	...
	if ((user_param->tst == BW || user_param->tst == LAT_BY_BW) && (user_param->machine == CLIENT || user_param->duplex)) {
		...
		ALLOCATE(ctx->scnt_now,uint64_t,user_param->num_of_qps);
		ALLOCATE(ctx->ccnt_now,uint64_t,user_param->num_of_qps);
		memset(ctx->scnt_now, 0, user_param->num_of_qps * sizeof (uint64_t));
		memset(ctx->ccnt_now, 0, user_param->num_of_qps * sizeof (uint64_t));
...
int ctx_init(struct pingpong_context *ctx, struct perftest_parameters *user_param)
{
  ...
  switch ((int)ctx->qp[0]->qp_context){
    case 0:
      printf("@@@ This is BW flow\n");
      break;
    case 1:
      printf("@@@ This is Delay flow\n");
      break;
    case 2:
      printf("@@@ This is Msg flow\n");
        break;
    default:
        printf("No Match!!!\n");
  }
...
struct ibv_qp* mt_perf_qp_create(struct pingpong_context *ctx,
		struct perftest_parameters *user_param, int i)
{
	...
	memset(&attr, 0, sizeof(struct ibv_qp_init_attr));
  // use different CQs
  int qp_idx = i/user_param->qps_in_thrd;
	attr.send_cq = ctx->mtperf_send_cq[qp_idx];
	attr.recv_cq = (user_param->verb == SEND) ? ctx->mtperf_recv_cq[qp_idx] : ctx->mtperf_send_cq[qp_idx];
	attr.cap.max_send_wr  = user_param->tx_depth;
	attr.cap.max_send_sge = MAX_SEND_SGE;
	attr.cap.max_inline_data = user_param->inline_size;
...
int run_iter_bw_infinitely(struct pingpong_context *ctx,struct perftest_parameters *user_param)
{
  ...
  int    mtqp_first_flag = 0;
  
  if (user_param->core_start >= 0){
    cpu_set_t mask;
    CPU_ZERO(&mask);
    CPU_SET(user_param->core_start, &mask);
    if (sched_setaffinity(0, sizeof(mask), &mask)){
      fprintf(stderr, "Failed to set affinity on main thread!\n");
      return FAILURE;
    }
  }
  ...
  if (user_param->core_start >= 0){
    cpu_set_t th_cpu;
    CPU_ZERO(&th_cpu);
    //CPU_SET(user_param->core_start, &th_cpu);
    CPU_SET(1, &th_cpu);
    if (pthread_setaffinity_np(print_thread, sizeof(th_cpu), &th_cpu)){
      fprintf(stderr, "Failed to set affinity on pthread!\n");
      return FAILURE;
    }
  }
  ...
  if (!mtqp_first_flag){
    // Presetting args
    mtqp_arg.ctx = ctx;
    mtqp_arg.user_param = user_param;
    mtqp_arg.return_value = 0;
  }

  // Time measuring starts
  gettimeofday(&ctx->start_t, NULL);

  int qp_per_thrd = user_param->qps_in_thrd;
  int* idx_list, idx_len=0;

	/* main loop for posting */
	while (1) {
	/* main loop to run over all the qps and post each time n messages */
    if (user_param->num_of_qps > 1 && user_param->qp_thrds != NULL){
 
      // Launch Posting Threads from idx 1 ~
      if (!mtqp_first_flag){
        mtqp_first_flag = 1;

        // Assign certain number of QPs to One Thread
        //if (user_param->num_of_qps <= 8){
        if (qp_per_thrd >= 1){

          int num_thrd = user_param->num_of_qps / qp_per_thrd - 1; 
          
          /*
          if (user_param->num_of_qps % qp_per_thrd)
            num_thrd++;
          */

          printf("\n<<< QPS IN THREAD MODE >>>\n"); 
          //printf("MAX QPs possibly assigned to each thread: %d\n", qp_per_thrd);
          //printf("Total Threads needed: %d\n", num_thrd);
          //if (user_param->core_end - user_param->core_start + 1 < num_thrd)
            //printf("    (** Performance might be low. Threads Pinned(%d) < Thread Required(%d)\n", user_param->core_end - user_param->core_start + 1, num_thrd);

          //printf("Core #%d ----> Thread Main\n", user_param->core_start);
          int cur_core_idx = user_param->core_start + 1;  // first core is assigned to main thread for logging & completion!
          for (int index = 0 ; index < num_thrd ; index++) {
         
           // get idx_list length 
            idx_len = user_param->num_of_qps / num_thrd;
            
            /*
            int residue = user_param->num_of_qps % num_thrd;
            if (residue) idx_len++;
            */

            //printf("idx len : %d\n", idx_len);
            idx_list = (int*) malloc(sizeof(int) * idx_len);
            memset(idx_list, -1, sizeof(int)* idx_len);

            // get index list
            for (int k = 0; k < idx_len; k++){
              int cur_qp = idx_len * index + k;
              //printf("Current QP idx = %d\n", cur_qp);
              if (cur_qp >= user_param->num_of_qps){
                printf("QP idx(%d) exceeded total QP num(%d)! break.\n", cur_qp, user_param->num_of_qps);
                break;
              }
              idx_list[k] = cur_qp;
            }

            /*printf("Core #%d ----> QP [ ", cur_core_idx);
              for (int j=0; j<num_thrd; j++)
              printf("%d ", idx_list[j]);
              printf(" ]\n");*/

            struct mtqp *arg = (struct mtqp*) malloc(sizeof(struct mtqp));
            memcpy(arg, &mtqp_arg, sizeof(struct mtqp));

            arg->idx_lst = idx_list;
            arg->idx_len = idx_len;
            arg->index = index;
            arg->qps_in_thrd = qp_per_thrd;

            pthread_create(&user_param->qp_thrds[index], NULL, mtqp_post2, arg);


            if (user_param->core_start >= 0){
              cpu_set_t th_cpu;
              CPU_ZERO(& th_cpu);
              CPU_SET(cur_core_idx++, &th_cpu);
              if (cur_core_idx > user_param->core_end)
                cur_core_idx = user_param->core_start + 1;

              if (pthread_setaffinity_np(user_param->qp_thrds[index], sizeof(th_cpu), &th_cpu)){
                fprintf(stderr, "Failed to set affinity on pthread!\n");
                return FAILURE;
              }
            }
          ...
          // Every Thread does posting for all QPs
          int cores_avail = user_param->core_end - user_param->core_start;
          //int num_thrd = cores_avail >= user_param->num_of_qps ? user_param->num_of_qps : cores_avail;
          int num_thrd = cores_avail;
          ...
          idx_list = (int*) malloc(sizeof(int) * user_param->num_of_qps);

          mutex_arr = (pthread_mutex_t*)malloc(sizeof(pthread_mutex_t) * user_param->num_of_qps);

          for(int k=0; k<user_param->num_of_qps; k++)
          {
            idx_list[k] = k;
            pthread_mutex_init(&(mutex_arr[k]), NULL);
          }
          //} 
          ...
          for (int index = 0 ; index < num_thrd ; index++) {
            ...
            // New thread
            pthread_create(&user_param->qp_thrds[index], NULL, mtqp_post, arg);
      ...
      if (qp_per_thrd >= 1) {
        int index = user_param->num_of_qps - 1;
        while ((ctx->scnt[index] - ctx->ccnt[index]) < user_param->tx_depth) {
          if (user_param->post_list == 1 && (ctx->scnt[index] % user_param->cq_mod == 0 && user_param->cq_mod > 1)) {
        ...
        for (int idx =0 ; idx < user_param->num_of_qps ; idx++) {
          ...
          if (totccnt < totscnt){
            ...
            if (ne > 0) {
              ...
              user_param->iters += user_param->cq_mod * ne;
...
int run_iter_lat_write_infinitely(struct pingpong_context *ctx,struct perftest_parameters *user_param)
{
	uint64_t                scnt = 0;
	uint64_t                ccnt = 0;
	//uint64_t                rcnt = 0;
  ...
  pthread_t print_thread;
	if (pthread_create(&print_thread, NULL, &handle_signal_print_thread2,(void *)&set) != 0){
		printf("Fail to create thread \n");
		return FAILURE;
	}

  catch_alarm_infinitely2();
	/* Done with setup. Start the test. */
	while (scnt < user_param->iters || ccnt < user_param->iters /*|| rcnt < user_param->iters*/
			|| ((user_param->test_type == DURATION && user_param->state != END_STATE))) {
...
int run_iter_lat_write_infinitely_rand(struct pingpong_context *ctx,struct perftest_parameters *user_param)
{
	uint64_t                scnt = 0;
	uint64_t                ccnt = 0;
	int                     ne;
  ...
  srand(MING_SEED);
	...
	while (scnt < user_param->iters || ccnt < user_param->iters /*|| rcnt < user_param->iters*/
        ...
        //user_param->latency_gap = rand() % 101; // 0~100us
        //user_param->latency_gap = 1000; // 0~100us
	 			total_gap_cycles = user_param->latency_gap * cpu_mhz;
				start_gap = get_cycles();
				end_cycle = start_gap + total_gap_cycles;
				while (get_cycles() < end_cycle) {
					continue;
			...
			*post_buf = (char)++scnt;
			#ifdef HAVE_VERBS_EXP
			if (user_param->use_exp == 1){
		    ctx->exp_wr[0].sg_list->length = rand() % 121 + 8;  // 8~128 bytes
				err = (ctx->exp_post_send_func_pointer)(ctx->qp[0],&ctx->exp_wr[0],&bad_exp_wr);
      } else {
		    ctx->wr[0].sg_list->length = rand() % 121 + 8;
				err = (ctx->post_send_func_pointer)(ctx->qp[0],&ctx->wr[0],&bad_wr);
      }
			#else
			err = post_send_method(ctx, 0, user_param);
...
int run_iter_lat_send(struct pingpong_context *ctx,struct perftest_parameters *user_param)
{
	...
	uintptr_t		primary_send_addr = ctx->sge_list[0].addr;
	uintptr_t		primary_recv_addr = ctx->recv_sge_list[0].addr;
	if (user_param->connection_type != RawEth && user_param->machine == CLIENT) {
		#ifdef HAVE_VERBS_EXP
   		if (user_param->use_exp == 1) {
			ctx->exp_wr[0].sg_list->length = user_param->size;
...
void catch_alarm_infinitely2()
{
	...
	duration_param->tposted[0] = get_cycles();
		...
		case SAMPLE_STATE:
			...
			duration_param->tcompleted[0] = get_cycles();
			get_cpu_stats(duration_param,2);
	    print_report_lat_duration(duration_param);
      catch_alarm_infinitely2();  // go to START_STATE
			break;
		default:
			fprintf(stderr,"unknown state\n");
	}
...
void *handle_signal_print_thread2(void *sigmask)
{
	sigset_t *set = (sigset_t*)sigmask;
	int rc;
	int sig_caught;
