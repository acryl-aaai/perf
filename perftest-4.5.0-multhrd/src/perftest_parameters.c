...
static void init_perftest_params(struct perftest_parameters *user_param)
{
  ...
  user_param->qp_thrds = NULL;
  user_param->qps_in_thrd = 1;
  user_param->print_per_qp = 0;
  user_param->core_start = -1;
  user_param->core_end = -1;
  user_param->app_id = -1;
...
static void force_dependecies(struct perftest_parameters *user_param)
{
	...
	if (user_param->test_method == RUN_INFINITELY) {
		...
		if (user_param->tst == LAT) {
			printf(RESULT_LINE);
			fprintf(stderr," run_infinitely for Latency mode!\n");
			//exit(1);
		}
...
int parser(struct perftest_parameters *user_param,char *argv[], int argc)
{
  ...
  static int thrd_per_qp_flag = 0;
  static int qps_in_thrd_flag = 0;
  static int print_per_qp_flag = 0;
  static int core_pinning_flag = 0;
  static int app_id_flag = 0;
	...
	while (1) {
		static const struct option long_options[] = {
			...
			#endif
			{ .name = "thrd_per_qp",		.has_arg = 0, .flag = &thrd_per_qp_flag, .val = 1},
			{ .name = "qps_in_thrd",		.has_arg = 1, .flag = &qps_in_thrd_flag, .val = 1},
			{ .name = "print_per_qp",		.has_arg = 0, .flag = &print_per_qp_flag, .val = 1},
			{ .name = "core_pinning",		.has_arg = 1, .flag = &core_pinning_flag, .val = 1},
			{ .name = "app_id",		      .has_arg = 1, .flag = &app_id_flag, .val = 1},
		...
		switch (c) {
          ...
          if (size_len == 0)
            user_param->lat_rand_flag = 1;

				  if (user_param->size < 0 || user_param->size > (UINT_MAX / 2)) {
        ...
        if(qps_in_thrd_flag) {
          user_param->qps_in_thrd = strtol(optarg, NULL, 0);
          if (user_param->qps_in_thrd < 0){
            fprintf(stderr, "--qps_in_thrd should not get negative value.\n");
						return FAILURE;
          }
          qps_in_thrd_flag = 0;
        }
 
        if(core_pinning_flag) {
          // Parse: split by '-'
          if (strlen(optarg) >= 3){
          user_param->core_start = strtol(strtok(optarg, "-"), NULL, 0);
          optarg = strtok(NULL, "-");
          user_param->core_end = strtol(strtok(optarg, "-"), NULL, 0);
          } else {
            user_param->core_start = strtol(optarg, NULL, 0);
            user_param->core_end = user_param->core_start;
          }

          printf("start: %d  end: %d\n", user_param->core_start, user_param->core_end); 
          if (user_param->core_start < 0 || user_param->core_end < 0 || user_param->core_start > user_param->core_end){
            fprintf(stderr, "Check --core_pinning values. (usage: --core_pinning=8-12");
  						return FAILURE;
          }
          core_pinning_flag = 0;
        }
        ...
        if(app_id_flag) {
          user_param->app_id = strtol(optarg, NULL, 0);
  ...
  if(thrd_per_qp_flag){
    printf("############# Thread per QP enabled. ############\n");
    ALLOCATE(user_param->qp_thrds, pthread_t, user_param->num_of_qps);

    printf("---> Number of QPs in each Thread: %d\n", user_param->qps_in_thrd);

  } else if (qps_in_thrd_flag) {
		fprintf(stderr," --qps_in_thrd option is available only when --thrd_per_qp is enabled.\n");
    return 1;
  }

  if(print_per_qp_flag){
    printf("############# Print per QP enabled. ############\n");
    user_param->print_per_qp = 1;
  }
...
void print_full_bw_report (struct perftest_parameters *user_param, struct bw_report_data *my_bw_rep, struct bw_report_data *rem_bw_rep)
{
  ...
  {
    if (user_param->app_id < 0)
  		printf( inc_accuracy ? REPORT_FMT_EXT : REPORT_FMT, my_bw_rep->size, my_bw_rep->iters, bw_peak, bw_avg, msgRate_avg);
    else
      printf(" %-7lu    %" PRIu64 "           %-7.6lf            %-7.6lf            %-7.6lf     %-5d", my_bw_rep->size, my_bw_rep->iters, bw_peak, bw_avg, msgRate_avg, user_param->app_id);
  }
